...
#pragma omp parallel for default(none),schedule(SCHEDULETYPE, 1),shared(Bodies, bi),private(i),reduction(+:fx,fy,fz)
                ...
                #pragma omp parallel for default(none),schedule(SCHEDULETYPE, 1),shared(Bodies)
                 for( int i = 0; i < NUMBODIES; i++ )
    ...
    double avgMegaMults = sumMegaBodies/(double)NUMTRIES;
