...
//  
//
//  Created by Kevin Lewis on 4/3/18.
//
...
int main( int argc, char *argv[ ] )
{
    ...
    fprintf( stderr, "Using %d threads and %d Nodes\n", NUMT, NUMNODES );
    ...
    for(j = 0; j < NUMTRIES; j++)
    {
        ...
        for( int i = 0; i < NUMNODES*NUMNODES; i++ )
        {
            ...
            // If we are at the top or the bottom
            ...
            // The corners have been ruled out, only check if we are at an edge
            else if(iv == 0 || iv == (NUMNODES - 1))
            {
                weightFactor = 0.5;
            }
            // The only other option is that we have a full piece
            else
            {
                weightFactor = 1.0;
            }
            ...
            sumVolume += height * weightFactor;
        }
        
        //printf("SumVolume is %f\n", sumVolume *= fullTileArea);
        
        double time1 = omp_get_wtime( );
        double megaHeights = (double)(NUMNODES * NUMNODES)/(time1-time0)/1000000.;
        sumMegaMults += megaHeights;
        if( megaHeights > maxMegaMults )
        {
            maxMegaMults = megaHeights;
        }
